...
You can return any solution in any order and the same applies for IDs. Also, it is guaranteed that there exists at least
one solution.
...
def groupThePeople(groupSizes):
    num_of_people = len(groupSizes)
    group_dict = {}
    result = []
    for i in range(num_of_people):
        if groupSizes[i] in group_dict:
            if len(group_dict[groupSizes[i]][-1]) == groupSizes[i]:
